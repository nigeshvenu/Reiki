...
class HomePageVM: NSObject {
    ...
    func getAllUserCards(urlParams:[String:Any]?,param:[String:Any]?,onSuccess: @escaping (String) -> Void, onFailure: @escaping (String) -> Void){
        let function = APIFunction.user_card
        RequestManager.serverRequestWithToken(function: function, method: .get, urlParams: urlParams, parameters: param, onSuccess: { result in
               if let error = result["error"] as? String{
                   if !(error.isEmpty){
                       onFailure(error)
                ...
                print(result)
                if let data = result["data"] as? [String:Any]{
                    if let userCoins = data["user_cards"] as? [[String:Any]]{
                        if userCoins.count == 0{
                            self.isPageEmptyReached = true
                        }else{
                            for i in userCoins{
